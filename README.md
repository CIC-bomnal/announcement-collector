...
지정한 검색어/금지어를 설정하면 해당 키워드로 나라장터(조달청)와 K-Startup(창업진흥원)의 공고를 매일 자동으로 검색하여 Google 스프레드시트에 쌓아주는 코드입니다.
...
셋 다 무료이고 결제 정보는 필요 없습니다. 세팅에 걸리는 시간은 약 20분입니다.

> GitHub 없이 쓰고 싶다면 Code 버튼 → Download ZIP 으로 받아서 6단계까지만 진행하면 됩니다. 대신 매일 자동 실행은 되지 않으니, PC 의 작업 스케줄러에 `python main.py` 를 등록해 두어야 합니다. 그 시각에 PC 가 켜져 있어야 하므로 GitHub 방식을 권합니다.
...
3. Owner 는 본인 계정(또는 조직 계정), Repository name 은 자유롭게 정합니다. 검색어가 외부에 보이는 게 싫다면 **Private** 을 선택하세요. Private 이어도 무료 플랜의 GitHub Actions 한도(월 2,000분)로 충분합니다.
...
> 회사 Google Workspace 에서 "서비스 계정 키 생성이 조직 정책으로 차단" 이라고 나오면 관리자에게 예외를 요청하거나, 개인 Google 계정으로 프로젝트를 만들어도 됩니다. 스프레드시트만 공유하면 되므로 어느 계정의 프로젝트든 상관없습니다.
...
금지어는 이 판정과 별개로, 수집된 결과에서 제목에 금지어가 있는 공고를 빼서 **(필터)** 탭을 만듭니다. 원본 탭에는 그대로 남습니다.

> **고급: `must` 검색어.** 핵심 단어와 금지어가 한 제목에 같이 오는 경우(예: "운영기관 모집 및 참가기업 모집")를 놓치고 싶지 않다면 `config.yaml` 의 주석 처리된 `must` 항목을 켜세요. `must` 단어가 있는 공고는 금지어가 있어도 (필터) 탭에 남습니다.
